...
static ssize_t write_to_kernel(struct file *file, const char __user *user_buffer, size_t count, loff_t *ppos)
{
    ...
    // copy_from_user returns the number of bytes that could not be copied. On success, this will be zero.
    if (copy_from_user(kernel_buffer, user_buffer, min(count, MAX_MESSAGE_SIZE)))
        return -EFAULT; // Return an error if the copy was not successful
...
static ssize_t read_from_kernel(struct file *file, char __user *user_buffer, size_t count, loff_t *ppos)
{
    ...
    // Hint: Only one line of code is needed
    retval = min(count, len - *ppos);

    // TODO 3: copy from the kernel buffer to user buffer by using copy_to_user()
    // Hint: Only one line of code is needed
    // copy_to_user returns the number of bytes that could not be copied. On success, this will be zero.
